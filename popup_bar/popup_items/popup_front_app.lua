...
M.front_app = sbar.add("item", "front_app_popup", {
	display = "active",
	position = "center",
	y_offset = 20,
	icon = { drawing = false },
...
M.front_app:subscribe("front_app_switched", function(env)
	sbar.animate("tanh", 40, function()
		M.front_app:set({ label = { string = env.INFO }, y_offset = 50 })
		M.front_app_bracket:set({ y_offset = 40 })
		M.front_app:set({ y_offset = -10 })
		M.front_app_bracket:set({ y_offset = -10 })
		M.front_app:set({ y_offset = 50 })
		M.front_app_bracket:set({ y_offset = 40 })
	end)
end)
